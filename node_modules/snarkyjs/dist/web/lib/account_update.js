...
// internal API
export { smartContractContext, Permission, Preconditions, Body, Authorization, ZkappCommand, addMissingSignatures, addMissingProofs, signJsonTransaction, ZkappStateLength, Events, Actions, TokenId, Token, CallForest, createChildAccountUpdate, zkAppProver, };
...
function addMissingSignatures(zkappCommand, additionalKeys = []) {
    ...
    let { commitment, fullCommitment } = Ledger.transactionCommitments(JSON.stringify(ZkappCommand.toJSON(zkappCommand)));
...
function signJsonTransaction(transactionJson, privateKey) {
    ...
    for (let i = 0; i < zkappCommand.accountUpdates.length; i++) {
        ...
        if (accountUpdate.body.publicKey === publicKey &&
            ...
            zkappCommand = JSON.parse(Ledger.signOtherAccountUpdate(JSON.stringify(zkappCommand), privateKey, i));
        }
    }
